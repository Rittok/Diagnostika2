...
def primary_diagnostic_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            correct_answers = sum(result.values())  # True считается как 1, False как 0
            percent_correct = (correct_answers / len(questions)) * 100
            
            # Передаем результат в представление результатов
            return redirect('diagnostic_results', percent=percent_correct)
    else:
...
def diagnostic_results(request, percent):
    """Отображает результаты первичного тестирования."""
    context = {
